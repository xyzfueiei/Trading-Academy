...
function CoursesAdmin({ rows, reload, remove, setMessage, reorder }) {
  // Keep the course editor aligned with the existing courses table schema.
  // The schema does not contain a `level` column, so level must not be sent
  // in Supabase inserts/updates or referenced as an admin form field.
  const blank = { id: null, title: '', description: '', display_order: rows.length + 1, is_published: true }
  const [form, setForm] = useState(blank)
  const editing = Boolean(form.id)
  async function save(event) {
    event.preventDefault()
    if (!form.title.trim()) return setMessage({ tone: 'error', text: 'Course title is required.' })
    const payload = { title: form.title.trim(), description: form.description.trim(), display_order: Number(form.display_order) || 1, is_published: Boolean(form.is_published) }
  ...
  return <>
    <div className="admin-panel"><AdminHeader title={editing ? 'Edit course' : 'Create a course'} count="Course catalogue" /><form className="admin-form-grid" onSubmit={save}><Field label="Title" wide value={form.title} onChange={(value) => setForm({ ...form, title: value })} required /><Field label="Display order" type="number" value={form.display_order} onChange={(value) => setForm({ ...form, display_order: value })} /><TextField label="Description" wide value={form.description} onChange={(value) => setForm({ ...form, description: value })} /><label className="field checkbox-field"><span>Published to learners</span><input type="checkbox" checked={form.is_published} onChange={(event) => setForm({ ...form, is_published: event.target.checked })} /></label><div className="admin-form-actions"><Button type="submit">{editing ? <><Save size={14} /> Save course</> : <><Plus size={14} /> Create course</>}</Button>{editing && <Button type="button" variant="outline-dark" onClick={() => setForm({ ...blank, display_order: rows.length + 1 })}>Cancel</Button>}</div></form></div>
    <div className="admin-panel"><AdminHeader title="Course catalogue" count={`${rows.length} total`} />{rows.length ? <TableWrap><table className="admin-table"><thead><tr><th>Course</th><th>Status</th><th>Order</th><th>Actions</th></tr></thead><tbody>{rows.map((row, index) => <tr key={row.id}><td><strong>{row.title}</strong><br /><small>{(row.description || '').slice(0, 90)}</small></td><td><Badge tone={row.is_published ? 'success' : 'muted'}>{row.is_published ? 'published' : 'draft'}</Badge></td><td>{row.display_order ?? '—'}</td><td><RowActions onEdit={() => setForm({ ...blank, ...row, description: row.description || '' })} onToggle={() => toggle(row)} toggleLabel={row.is_published ? 'Unpublish' : 'Publish'} onUp={() => reorder('courses', rows, index, 'up')} onDown={() => reorder('courses', rows, index, 'down')} onDelete={() => remove('courses', row.id)} /></td></tr>)}</tbody></table></TableWrap> : <EmptyState title="No courses" body="Create the first course above." />}</div>
  </>
}
